package com.ispong.oxygen.core;

import com.ispong.oxygen.core.config.OxygenCoreAutoConfiguration;
import com.ispong.oxygen.core.pojo.Dog;
import com.ispong.oxygen.core.secret.AesUtils;
import com.ispong.oxygen.core.secret.JwtUtils;
import com.ispong.oxygen.core.secret.RsaUtils;
import org.junit.jupiter.api.Test;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.test.context.ActiveProfiles;
import org.springframework.test.context.ContextConfiguration;

@SpringBootTest
@ActiveProfiles
@ContextConfiguration(classes = OxygenCoreAutoConfiguration.class)
public class CoreSecretTasks {

    private final String key = "test";

    @Test
    public void testAes() {

        String data = AesUtils.encrypt(key, "love you");
        System.out.println("data-->" + AesUtils.decrypt(key, data));
    }

    @Test
    public void testJwt() {

        Dog dog = new Dog("wang", 12);
        String data = JwtUtils.encrypt(key, dog);
        System.out.println("data-->" + JwtUtils.decrypt(key, data, Dog.class).toString());
    }

    @Test
    public void testRsa(){

        String publicKey = "";
        String privateKey = "";
        String data = RsaUtils.encrypt(publicKey, "i love you");
        System.out.println("data-->" + RsaUtils.decrypt(privateKey, data));
    }

}
